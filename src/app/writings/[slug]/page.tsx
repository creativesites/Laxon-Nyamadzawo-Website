...
export default function ArticlePage() {
  ...
  const renderContent = (raw: string) => {
    ...
    lines.forEach((line, i) => {
      const t = line.trim();
      if (t.startsWith("## ")) { flush(i); elements.push(<h2 key={i} className="font-serif text-2xl md:text-3xl font-bold text-charcoal mt-14 mb-5 pb-3 border-b border-gold-200">{t.substring(3)}</h2>); }
      ...
      else if (/^\d+\.\s/.test(t)) { flush(i); elements.push(<p key={i} className="flex gap-3 my-3 text-gray-600 leading-relaxed"><span className="text-gold-500 font-bold">•</span><span dangerouslySetInnerHTML={{ __html: inline(t.replace(/^\d+\.\s/, "")) }} /></p>); }
  ...
  if (loading) return (<><Navbar /><div className="min-h-screen flex items-center justify-center"><div className="w-10 h-10 border-4 border-gold-200 border-t-gold-500 rounded-full animate-spin" /></div><Footer /></>);
    ...
    <>
      ...
      <section className="relative pt-32 pb-16 bg-charcoal">
        <div className="max-w-3xl mx-auto text-center px-6">
          <span className="inline-block px-4 py-1.5 bg-gold-500 text-white text-[10px] font-semibold uppercase tracking-widest rounded mb-6">{writing.category}</span>
      ...
      <main className="py-16 px-6 lg:px-8">
        <article className="max-w-3xl mx-auto">
          <div className="flex items-center justify-between mb-10 pb-6 border-b border-gray-100">
            <Link href="/writings" className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-gold-600 hover:text-gold-700 transition-colors">← All Writings</Link>
            <button onClick={() => { navigator.clipboard.writeText(window.location.href); alert("Link copied!"); }} className="text-xs font-semibold uppercase tracking-wider text-gold-600 hover:text-gold-700 transition-colors cursor-pointer">Share →</button>
          </div>
          ...
          <div className="border-t border-gray-200 mt-16 pt-10 text-center space-y-3">
            <p className="font-serif italic text-gold-600 text-sm">&ldquo;To God be the glory, great things He has done.&rdquo;</p>
